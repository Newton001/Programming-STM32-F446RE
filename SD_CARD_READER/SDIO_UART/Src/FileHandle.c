...
{
	if (fresult != FR_OK) {
		sprintf(buffer, "Error in mounting SD CARD. Error code: %d\r\n", fresult);
		send_uart(buffer);

		// Provide additional diagnostic hints based on common errors
		switch (fresult) {
		case FR_DISK_ERR:
			send_uart("Low-level hardware error.\r\n");
			break;
		case FR_NOT_READY:
			send_uart("SD card is not ready. Check connection and initialization.\r\n");
			break;
		case FR_NO_FILESYSTEM:
			send_uart("No valid FAT volume found. Format the SD card properly.\r\n");
			break;
		case FR_INVALID_DRIVE:
			send_uart("Invalid drive number specified.\r\n");
			break;
		case FR_MKFS_ABORTED:
			send_uart("File system creation aborted.\r\n");
			break;
		default:
			send_uart("Unknown error occurred.\r\n");
			break;
		}
	} else {
